...
def main():
    ...
    if score == 30:
        print("Gratulacje, udało ci się zdobyć wszystkie punkty ! W nagrodę gra zapamięta twoje imię!")
        try:
            wyniki = open("wyniki.txt", "w+")
        except IOError as e:
            print("Wystąpił błąd, nie można otworzyć pliku", wyniki.txt, "!", e)
            input("\n\nAby zakończyć program wciśnij Enter")
            sys.exit()
        else:
            name = input("Jak masz na imię?")
        wynik = name + ": " + str(score)
        wyniki.write(wynik)
        print("Zapamiętane !", wynik)
        wyniki.close()
